...
if [[ "${PV}" == "9999" ]]; then
	...
	ZLIB_NG_COMMIT="cb43e7f"  # patched zlib-ng which is a replacement for zlib, not yet in portage
...
src_prepare() {
	...
	mysed '(find_program \(CCACHE_FOUND ccache\))' '# ebuild disabled: \1' cmake/find_ccache.cmake
	...
	mysed 'if \(USE_STATIC_LIBRARIES\)' 'if (MAKE_STATIC_LIBRARIES)' cmake/find_zlib.cmake
	...
	if use libcxx; then
		mysed '\$\{LIBCXXFS_LIBRARY}(.+)' '\1 # ebuild: fix libcxx-fs usage' cmake/find_cxx.cmake
		...
		use llvm && mysed 'if \(LLVM_FOUND AND OS_LINUX AND USE_LIBCXX\)' \
			'if(FALSE) # ebuild: system llvm build with system libc++' cmake/find_llvm.cmake
	fi
	...
	if use kafka; then
		mysed 'list *\(APPEND +RDKAFKA_LIBRARY +\$\{LZ4_LIBRARY\}\)' '' cmake/find_rdkafka.cmake
	...
	if use '!orc'; then
		mysed 'set\(USE_ORC +1\)' 'set(USE_ORC 0) # ebuild fix' cmake/find_orc.cmake
...
src_configure() {
	...
	mycmakeargs+=(
		...
		-D"USE_INTERNAL_RE2_LIBRARY"="$(usex static)"
		-D"USE_INTERNAL_ORC_LIBRARY"="$(usex orc)"
