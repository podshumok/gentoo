...
IUSE="
	...
	protobuf readline s3 +server split-binary ssl +static +static-libs test tools
"
...
if [[ "${PV}" == "9999" ]]; then
	...
	EGIT_SUBMODULES=( '*'
		...
		-contrib/curl			-contrib/double-conversion
		-contrib/icu			-contrib/icudata	-contrib/flatbuffers
		-contrib/googletest		-contrib/hyperscan
		-contrib/jemalloc		-contrib/libcxx		-contrib/libcxxabi
		-contrib/librdkafka		-contrib/libxml2	-contrib/llvm
		-contrib/openssl		-contrib/lz4		-contrib/mariadb-connector-c
		-contrib/protobuf		-contrib/unixodbc	-contrib/zstd
		'-contrib/*/*'
	)
	TYPE="unstable"
	S="${WORKDIR}/${P}"
	...
	# last check: 9d65152
	ARROW_COMMIT="b789226"   # not yet in portage (0.15.1)
	AWS_COMMIT="45dd855"  #  1.7.240
	AWS_C_COMMON_COMMIT="736a82d" # about v0.4.24
	AWS_C_EVENT_STREAM_COMMIT="3bc3366" # unstable, post 0.1.4, Nov 21, 2019
	AWS_CHECKSUMS_COMMIT="519d6d9" # v0.1.5
	BASE64_COMMIT="32a8722"  # not yet in portage (commit from Dec 2, 2019)
	...
	CURL_COMMIT="3b8bbbb"  #  unstable, post-7.67.0, commit from Dec 4, 2019
	...
	FLATBUFFERS_COMMIT="bf9eb67" # v1.11
	...
	ICU_COMMIT="faa2f9f"  # in-between 64 and 65
	ICUDATA_COMMIT="f020820"  #
	JEMALLOC_COMMIT="cd2931a"  # somewhere in-between 5.1.0 and 5.2.0
	LIBCHEADERS_COMMIT="cd82fd9"  #
	LIBCXX_COMMIT="f7c6323"  # some random snapshot from 10.0.0 (In-Progress)
	LIBCXXABI_COMMIT="7aacd45"  #
	...
	LLVM_COMMIT="778c297"  # stripped down tag llvmorg-9.0.0
	LZ4_COMMIT="3d67671"  # v1.9.2
	MARIADB_CONNECTOR_C_COMMIT="1801630"  # patched pre-3.1.0
	OPENSSL_COMMIT="3d67671"  # unstable/patched 1.1.1d after Dec 11, 2019
	...
	ZLIB_NG_COMMIT="5673222"  # patched zlib-ng which is a replacement for zlib, not yet in portage
...
PATCHES="
	...
	${FILESDIR}/zlib-ng-change-name-r1.patch
"
...
RDEPEND_BASE="
	...
	!static? (
		...
		llvm? (
			...
			readline? ( >=sys-devel/llvm-8.0.0:=[ncurses] )
		)
		mysql? (
			|| (
				mysql? (
...
BDEPEND="
	...
	parquet? ( >=dev-libs/flatbuffers-1.11.0 )
	|| (
		>=sys-devel/gcc-9.0
		>=sys-devel/clang-7.0
	)
...
CLICKHOUSE_DEPS=(
	...
	"aws			s3		dev-libs/aws-sdk-cpp[s3,ssl?,static-libs] \
		USE_INTERNAL_AWS_S3_LIBRARY=no		USE_AWS_S3		ENABLE_S3"
	# aws-c-common
	# aws-c-event-stream
	# aws-checksums
	...
	"croaring		-		- \
		USE_INTERNAL_CROARING_LIBRARY=yes	USE_INTERNAL_CROARING_LIBRARY"
	# TODO: curl
	...
	"flatbuffers		parquet		>=dev-libs/flatbuffers-1.1.0[static-libs] \
		USE_INTERNAL_FLATBUFFERS_LIBRARY=no	FLATBUFFERS_LIBRARY"
	# googletest
	...
	"icudata		icu		- \
		USE_INTERNAL_ICUDATA_LIBRARY=no		ICU_DATA_LIBRARY_RELEASE"
	...
	"libxml2		-		>=dev-libs/libxml2-2.9.8:=[static-libs] \
		USE_INTERNAL_LIBXML2_LIBRARY=no		LIBXML2_LIBRARY"
	"llvm			llvm		>=sys-devel/llvm-8.0.0:=[libedit?,static-libs(+)] \
	...
	"lz4			-		>=app-arch/lz4-1.9.2:=[static-libs] \
...
CLICKHOUSE_VENDORING_HARDCODED=(
	...
	USE_INTERNAL_THRIFT_LIBRARY=yes
...
pkg_pretend() {
	...
	elif [[ $(gcc-major-version) -lt 9 ]] && [[$(gcc-minor-version) -lt 1 ]]; then
		eerror "Compilation with gcc older than 9.1 is not supported"
...
src_prepare() {
	...
	if ! tc-ld-is-lld; then
		mysed '(.+CMAKE_EXE_LINKER_FLAGS\} -Wl,-no-pie\"\))' '#\1 # ebuild disabled: possible bug' CMakeLists.txt
	fi
	...
	if use '!static' && use jemalloc; then
		lib="\${JEMALLOC_LIBRARIES}"
	...
	if use parquet; then
		...
		mysed '(add_dependencies\(\$\{ARROW_LIBRARY} protoc\))' '# ebuild disable: \1' contrib/arrow-cmake/CMakeLists.txt
		mysed '(add_subdirectory\(\$\{FLAT.+)' 'find_package\(Flatbuffers REQUIRED\) # ebuild disable: \1' contrib/arrow-cmake/CMakeLists.txt
		mysed '\$\{FLATBUFFERS_COMPILER\}|flatc([ )])' 'flatbuffers::flatc\1' contrib/arrow-cmake/CMakeLists.txt
		if use '!static'; then
			mysed '\$\{FLATBUFFERS_LIBRARY\}' 'flatbuffers::flatbuffers_shared' contrib/arrow-cmake/CMakeLists.txt
		else
			mysed '\$\{FLATBUFFERS_LIBRARY\}' 'flatbuffers::flatbuffers' contrib/arrow-cmake/CMakeLists.txt
		fi
		mysed '(set\(PROTOB.+)' '# ebuild disable: \1' contrib/arrow-cmake/CMakeLists.txt

		mysed 'boost_system_internal boost_filesystem_internal boost_regex_internal' \
			'boost_system boost_filesystem boost_regex' contrib/arrow-cmake/CMakeLists.txt

		sed -i '1iset(ebuild_save_CMAKE_REQUIRED_LIBRARIES "${CMAKE_REQUIRED_LIBRARIES}")' cmake/find/parquet.cmake
		sed -i '1iset(ebuild_save_CMAKE_REQUIRED_INCLUDES "${CMAKE_REQUIRED_INCLUDES}")' cmake/find/parquet.cmake
		echo 'set(CMAKE_REQUIRED_LIBRARIES $ebuild_save_CMAKE_REQUIRED_LIBRARIES)' >> cmake/find/parquet.cmake
		echo 'set(CMAKE_REQUIRED_INCLUDES $ebuild_save_CMAKE_REQUIRED_INCLUDES)' >> cmake/find/parquet.cmake
	fi
	if use '!static'; then
	...
	if use '!static-libs'; then
		...
		mysed '(unset \(BUILD_SHARED_LIBS CACHE\))' '#\1 # ebuild: disabled' contrib/CMakeLists.txt
		mysed '(target_compile_definitions \(zlibstatic.+)' '#\1 # ebuild: disabled' contrib/CMakeLists.txt

		echo 'install(TARGETS zlib)' >> contrib/zlib-ng/CMakeLists.txt
	else
		mysed '(target_compile_definitions \(zlib .+)' '#\1 # ebuild: disabled' contrib/CMakeLists.txt
		mysed '(add_library\(zlib SHARED.+)' '#\1 # ebuild: disabled' contrib/zlib-ng/CMakeLists.txt
		mysed '(set_target_properties\(zlib .+\))' '#\1 # ebuild: disabled' contrib/zlib-ng/CMakeLists.txt
		mysed 'set\(ZLIB_INSTALL_LIBRARIES zlib zlibstatic\)' 'set\(ZLIB_INSTALL_LIBRARIES ""\)' contrib/zlib-ng/CMakeLists.txt
		mysed '(set_target_properties\(zlib PROPERTIES LINK_FLAGS)' 'message(Disabled " \1 "' contrib/zlib-ng/CMakeLists.txt 
	fi
